...
function validateParamsInMethod(name, param_s, paramName) {
  ...
  if (param_s.type && param_s.error_code) {
    ...
    for (let param_name in param_s) {
      ...
      if (typeof param_s[param_name].type === 'function')
        throw Error('#0008 (you need call fn type for create), error params in method: ' + name + ' - [' + param_name + ']');
  ...
  call: (name, user, param, type) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      .catch(err => {
        ...
        API.saveLog(name, err, user, param, {success: false, result: false}, type, request_id);
        return Promise.reject(err);
